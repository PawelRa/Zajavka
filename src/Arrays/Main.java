package Arrays;

import Arrays.Conditionals;
import Arrays.OperatorsExamples;

public class Main {
    public static void main(String[] args) {

        OperatorsExamples.doubleSum(3,5);
        OperatorsExamples.colors(6);
        OperatorsExamples.colors(72);
        OperatorsExamples.divisionBy3and7(9);
        OperatorsExamples.divisionBy3and7(28);
        OperatorsExamples.divisionBy3and7(21);
        OperatorsExamples.divisionBy3and7(100);
        Conditionals.loopYear(2100);
        Conditionals.loopYear(1971);
        Conditionals.loopYear(1988);
        Conditionals.maxNumber(2,3,1);
        Conditionals.evenOrOdd(30);
        Conditionals.numberOfDaysInTheMonth(9);
        Conditionals.numberOfDays(9);
    }
}